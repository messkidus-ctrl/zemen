...
const ADMIN_PHONE = '0945206759';
const ADMIN_PASSWORD = 'Zemen@2026';
...
let PAYMENT_INFO = { telebirrNumber: '0945206759', telebirrName: 'kidus' };
...
app.post('/api/admin/login', (req, res) => {
  const phone = String(req.body.phone || '').replace(/^\+/, '').trim();
  const password = String(req.body.password || '').trim();

  if (phone === ADMIN_PHONE && password === ADMIN_PASSWORD) {
    return res.json({ ok: true, admin: true, phone: ADMIN_PHONE });
  }
...
function startTelegramBot(){
  const TOKEN=process.env.BOT_TOKEN, GAME_URL=process.env.GAME_URL||'https://your-render-app.onrender.com';
  if(!TOKEN){console.log('ℹ️ No BOT_TOKEN — Telegram bot disabled until environment is configured.');return;}
